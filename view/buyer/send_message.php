...
if ($farm_id > 0) {
    // Seller is sending the message
    $sql_check .= " JOIN users u ON c.farmer_id = u.id
                    JOIN farms f ON u.id = f.owner_id
                    WHERE c.id = ? AND f.id = ?";
    $stmt_check = $conn->prepare($sql_check);
    $stmt_check->bind_param("ii", $chat_id, $farm_id);
} else {
    // Buyer is sending the message
    $sql_check .= " WHERE c.id = ? AND (c.buyer_id = ? OR c.farmer_id = ?)";
    $stmt_check = $conn->prepare($sql_check);
...
// Get chat data
$chat_data = $result_check->fetch_assoc();

// Determine if sender is buyer or farmer
$is_buyer = ($sender_id == $chat_data['buyer_id']);
